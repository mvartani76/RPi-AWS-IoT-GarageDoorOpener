    ...
    @IBAction func deleteCertificateButtonPressed(_ sender: AnyObject) {
        ...
        let okAction: UIAlertAction = UIAlertAction( title: "Delete", style: .default) { action -> Void in
            ...
            if certificateArn != "from-bundle" && certificateId != nil
            {
                ...
                iot.updateCertificate( updateCertificateRequest! ).continue({ (task) -> AnyObject? in
                    ...
                    if (task.exception == nil && task.error == nil)
                    {
                        ...
                        iot.detachPrincipalPolicy(detachPolicyRequest!).continue({ (task) -> AnyObject? in
                            if let error = task.error {
                                print("failed: [\(error)]")
                            }
                            if let exception = task.exception {
                            ...
                            if (task.exception == nil && task.error == nil)
                            {
                                ...
                                iot.deleteCertificate(deleteCertificateRequest!).continue({ (task) -> AnyObject? in
